...
class Camera extends Evented {
    ...
    /**
     * Returns the map's current field of view (FOV). The fov is the current angular angular width of the camera
     * viewport. A small FOV results in a "flatter" image, and a large FOV enlgarges the foreground.
     *
     * @memberof Map#
     * @returns The map's current field of view.
     * @see [Navigate the map with game-like controls](https://www.mapbox.com/mapbox-gl-js/example/game-controls/)
     */
    getFov(): number {
    ...
    /**
     * Sets the map's field of view (FOV). The fov is the current angular angular width of the camera
     * viewport. A small FOV results in a "flatter" image, and a large FOV enlgarges the foreground.
     *
     * Equivalent to `jumpTo({fov: fov})`.
     *
     * @memberof Map#
     ...
     * map.setBearing(90);
     */
    setFov(fov: number, eventData?: Object) {
        this.jumpTo({ fov: fov }, eventData);
        return this;
    }
    ...
    ): void | (CameraOptions & AnimationOptions) {
    ...
    _fitInternal(
        calculatedOptions?: CameraOptions & AnimationOptions,
        options?: AnimationOptions & CameraOptions,
        eventData?: Object
    ) {
    ...
    /**
     * Changes any combination of center, zoom, bearing, fov, and pitch, without
    ...
    jumpTo(options: CameraOptions, eventData?: Object) {
        ...
        if (fovChanged) {
            this.fire("perspective", eventData);
        }
     ...
     */
    easeTo(
        options: CameraOptions & AnimationOptions & { easeId?: string },
        eventData?: Object
    ) {
        ...
        this._ease(
            (k) => {
                ...
                const newCenter =
                    k === 1
                        ? center
                        : tr.unproject(from.add(delta.mult(u(s))).mult(scale));
                tr.setLocationAtPoint(
                    tr.renderWorldCopies ? newCenter.wrap() : newCenter,
                    pointAtOffset
                );
    ...
    Debug.run(() => {
        ...
        ["drag", "zoom", "rotate", "pitch", "move"].forEach((name) => {
            ...
            camera.on(`${name}start`, () => {
                assert(
                    !inProgress[name],
                    `"${name}start" fired twice without a "${name}end"`
                );
